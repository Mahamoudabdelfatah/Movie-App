...
const People = () => {
  ...
  const url = `https://api.themoviedb.org/3/trending/person/week?api_key=${api_key}&page=${page}`
  ...
  const filterdPeople = allPeople.filter((movie) =>
      ...
      {loading ? (
        ...
        <div className='mt-10 px-3'>
          ...
          <div className='container mx-auto grid gap-6 px-4  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 mt-5'>
            {filterdPeople?.length === 0 && search.trim() !== "" &&
          ...
          {/* Pagination  */}
          {filterdPeople.length > 0 &&
            <div className='flex justify-center mt-5 space-x-3'>
              {[1, 2, 3, 4, 5].map((num) => (
                <button
                  key={num}
                  onClick={() => getPeople(num)}
